...
const Star = () => (
  <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M12.7276 2.44418L14.4874 5.99288C14.7274 6.48687 15.3673 6.9607 15.9073 7.05143L19.0969 7.58575C21.1367 7.92853 21.6167 9.4206 20.1468 10.8925L17.6671 13.3927C17.2471 13.8161 17.0172 14.6327 17.1471 15.2175L17.8571 18.3125C18.4170 20.7623 17.1271 21.71 14.9774 20.4296L11.9877 18.6452C11.4478 18.3226 10.5579 18.3226 10.0079 18.6452L7.01824 20.4296C4.87847 21.71 3.57862 20.7522 4.13856 18.3125L4.84848 15.2175C4.97846 14.6327 4.74849 13.8161 4.32853 13.3927L1.84881 10.8925C0.388969 9.4206 0.858919 7.92853 2.89869 7.58575L6.08834 7.05143C6.61828 6.9607 7.25821 6.48687 7.49818 5.99288L9.25797 2.44418C10.2179 0.518607 11.7777 0.518607 12.7276 2.44418Z" fill="#FDC700" stroke="#FDC700" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
...
const DashboardShipping = () => {
  ...
  const rate = 3.9; // This seems to be a static rate, consider fetching it if dynamic
  const [shipmentData, setShipmentData] = useState([]);
  const [shippingScope, setShippingScope] = useState([]);
  const [lastRating, setLastRating] = useState(null); // State for the last rating
    ...
    const fetchNotifications = async () => {
      try {
        ...
        const formatted = res.data.data.data.map((notif, index) => {
          ...
          // Adjusting logic based on common notification types
          // Assuming:
          // type 1 or 2 might be related to 'Check' (e.g., approval/completion)
          // type 2 (if distinct from 1) might be about 'Shipments'
          // type 3 might be 'Message'
          if (notif.notificationType === 1) { // Example: Accepted Offer
            icon = Check;
            borderColor = "#21CF61";
            iconColor = "#21CF61";
          } else if (notif.notificationType === 2) { // Example: New Shipment Request
            icon = Shipments;
            borderColor = "#F9751C";
            iconColor = "#F9751C";
          } else if (notif.notificationType === 3) { // Example: New Message
            icon = Message;
            borderColor = "#21CF61"; // Or a different color for messages
            iconColor = "#204C80";
  ...
  useEffect(() => {
    const fetchShippingScopeCount = async () => {
      try {
        ...
        console.log("😶‍🌫️ Offers Status Count:", res.data.data);
  ...
  useEffect(() => {
    const fetchLastRating = async () => {
      try {
        ...
        console.log("✨✨✨✨✨✨✨✨✨ Last Rating:", res.data);
        setLastRating(res.data.data);
      } catch (error) {
        console.error("❌ Error fetching last rating:", error);
        setLastRating(null); // Set to null if there's an error or no data
      }
    };
                ...
                <div className="border border-[#CACED8] bg-[#F1F1F166] rounded-[20px] px-4 py-5">
                  ...
                  <div className="flex items-center mt-6">
                    <img
                      src={
                        lastRating.imageUrl
                          ? (lastRating.imageUrl.startsWith('http')
                            ? lastRating.imageUrl
                            : `http://localhost:5092${lastRating.imageUrl}`) // Updated image loading logic
